...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        if (typeof window !== 'undefined' && typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
...
function WelcomeScreen({ onCreateGame, onJoinGame, existingGame }) {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen relative overflow-y-auto pb-10 bg-slate-950 px-6">
      <div className="w-full flex flex-col items-center justify-center mt-12 mb-8 text-center space-y-4 animate-in fade-in slide-in-from-top-10 duration-700">
      ...
      <div className="w-full max-w-sm">
        {mode === 'menu' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8">
            {existingGame && (
              <div className="bg-slate-900 border border-teal-500/30 p-4 rounded-2xl text-center mb-6">
            ...
            <button onClick={() => setMode('create')} className="w-full bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-3 active:scale-95 transition-all shadow-xl hover:bg-slate-800">
              <PlusCircle className="text-teal-400 w-8 h-8" />
              <span className="font-bold text-white uppercase tracking-widest">Yeni Oyun Kur</span>
            </button>
            <button onClick={() => setMode('join')} className="w-full bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-3 active:scale-95 transition-all shadow-xl hover:bg-slate-800">
              <LogIn className="text-fuchsia-400 w-8 h-8" />
              <span className="font-bold text-white uppercase tracking-widest">Oyuna Katıl</span>
            </button>
          </div>
        )}
        ...
        {mode === 'create' && (
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl animate-in zoom-in-95 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center mb-2">
                <h3 className="text-white font-bold text-xl font-black">Oyun Ayarları</h3>
            ...
            <div className="bg-slate-950 p-1 rounded-xl border border-slate-800 flex gap-1">
                <button 
                  ...
                  className={`flex-1 py-3 text-[9px] font-black rounded-lg transition-all uppercase tracking-widest ${hostRole==='banker'?'bg-slate-800 text-white shadow-lg border border-slate-700':'text-slate-600'}`}
                >
                  Bankacı
                </button>
                <button 
                  onClick={() => setHostRole('both')} 
                  className={`flex-1 py-3 text-[9px] font-black rounded-lg transition-all uppercase tracking-widest ${hostRole==='both'?'bg-slate-800 text-white shadow-lg border border-slate-700':'text-slate-600'}`}
                >
                  Oyuncu + Banka
                </button>
             </div>
            ...
            {hostRole === 'both' && (
              <div className="animate-in slide-in-from-top-2">
                <label className="text-[10px] text-slate-500 font-bold mb-2 block ml-1 uppercase">Piyon Seçin</label>
                <TokenSelector />
              </div>
            )}
        ...
        {mode === 'join' && (
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl animate-in zoom-in-95 shadow-2xl">
            ...
            <div className="mb-4">
                <label className="text-[10px] text-slate-500 font-bold ml-1 mb-1 block uppercase font-black">Oyun Kodu</label>
                <input 
            ...
            <div className="mb-6">
                <label className="text-[10px] text-slate-500 font-bold ml-1 mb-1 block uppercase font-black">Oyuncu Adı</label>
                <input 
                  type="text" value={name} onChange={e => setName(e.target.value)}
                  placeholder="İsim giriniz..."
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-teal-500 outline-none transition-colors font-bold"
                />
            </div>
            
            <label className="text-[10px] text-slate-500 font-bold mb-2 block ml-1 uppercase font-black">Piyon Seçimi</label>
            <TokenSelector />
            
            <button 
              onClick={() => onJoinGame(code, name, selectedToken)}
              disabled={!name || !selectedToken || code.length < 4}
              className="w-full bg-fuchsia-600 text-white font-bold py-4 rounded-xl shadow-lg active:scale-95 uppercase tracking-widest font-black"
            >
              GİRİŞ YAP
            </button>
          </div>
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-slate-950/95 backdrop-blur-md border-t border-slate-800 pb-safe z-40 h-20 shadow-2xl">
        <div className="max-w-md mx-auto flex justify-around p-2 gap-2">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 flex flex-col items-center justify-center gap-1 py-1 rounded-xl transition-all duration-300 ${activeTab === tab.id ? (tab.highlight ? 'text-amber-900 bg-gradient-to-tr from-amber-400 to-yellow-200 shadow-lg -translate-y-2 border-2 border-yellow-100 scale-105' : 'text-white bg-slate-800 border border-slate-600 shadow-inner font-bold') : (tab.highlight ? 'text-amber-500 bg-amber-950/30 border border-amber-900/50' : 'text-slate-500')}`}>
              {tab.icon}
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 pb-20">
      <div className={`relative overflow-hidden rounded-[2.5rem] p-8 border shadow-2xl mb-8 bg-slate-900 border-slate-800 text-center ring-1 ring-slate-700/30 shadow-inner`}>
        ...
        <span className="text-slate-500 text-[10px] tracking-[0.25em] mb-3 block font-bold uppercase font-black tracking-widest">Nakit Bakiyem</span>
      ...
      {!mode ? (
        <>
            <div className="grid grid-cols-2 gap-4">
              ...
              <button type="button" onClick={() => setMode('pay_player')} className="flex flex-col items-center justify-center bg-slate-900 border border-fuchsia-900/30 p-8 rounded-[2rem] gap-4 active:scale-95 group shadow-xl"><div className="p-4 bg-fuchsia-500/10 rounded-full text-fuchsia-500 border border-fuchsia-500/20 group-hover:scale-110 transition-transform shadow-inner"><Users size={32} /></div><span className="font-bold text-lg text-fuchsia-200 uppercase tracking-widest font-black">Oyuncu</span></button>
            </div>
            ...
            {showBankruptcyModal && (
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-6"><div className="w-full max-w-sm bg-slate-900 border-2 border-red-600 rounded-3xl p-8 text-center shadow-2xl"><Skull className="w-16 h-16 text-red-600 mx-auto mb-4" /><h2 className="text-3xl font-black text-white mb-2 uppercase font-black tracking-tighter">İflas</h2><p className="text-slate-400 text-sm mb-8 leading-relaxed font-bold">Varlıkları devredip oyundan ayrılacaksınız.</p><select value={beneficiary} onChange={(e) => setBeneficiary(e.target.value)} className="w-full bg-slate-950 border border-slate-700 text-white p-4 rounded-xl mb-6 outline-none focus:border-red-500 font-bold"><option value="">Kime Devredilsin?</option><option value="BANKA">BANKA</option>{players.filter(p => p.id !== user.id && !p.isBankerOnly).map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}</select><button type="button" onClick={() => beneficiary && handleBankruptcy(user.id, beneficiary)} className="w-full bg-red-600 text-white font-bold py-4 rounded-xl shadow-lg active:scale-95 mb-4 uppercase tracking-widest font-bold">Onayla</button><button type="button" onClick={() => setShowBankruptcyModal(false)} className="text-slate-500 font-bold uppercase text-xs tracking-widest hover:text-white font-bold">İptal</button></div></div>
            )}
        </>
      ) : (
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 shadow-2xl animate-in zoom-in-95 ring-1 ring-slate-700/30 shadow-inner font-bold">
          <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4 uppercase tracking-widest font-black"><h3 className="text-white font-bold text-xl flex items-center gap-2 uppercase tracking-widest font-bold">{mode === 'pay_bank' ? <Landmark className="text-red-500"/> : <Users className="text-fuchsia-500"/>} {mode === 'pay_bank' ? 'Banka Ödeme' : 'Para Gönder'}</h3><button type="button" onClick={() => setMode(null)} className="bg-slate-800 hover:bg-slate-700 text-slate-300 p-2 rounded-full transition-colors"><XCircle size={20}/></button></div>
          {mode === 'pay_player' && <div className="grid grid-cols-3 gap-3 mb-8 uppercase tracking-widest font-black">{players.filter(p => p.id !== user.id && !p.isBankerOnly).map(p => (<button key={p.id} type="button" onClick={() => setTargetId(p.id)} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 text-xs transition-all ${targetId === p.id ? 'bg-fuchsia-600 border-fuchsia-400 text-white shadow-lg scale-105' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-600'}`}><div className="w-12 h-12 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden">{(() => { const token = GAME_TOKENS.find(t => t.id === p.token); return token?.img ? <img src={token.img} alt="" className="w-full h-full object-cover" /> : token?.icon; })()}</div><span className="font-bold truncate w-full text-center tracking-tight uppercase font-bold">{p.name}</span></button>))}</div>}
          <div className="bg-slate-950 p-6 rounded-3xl border border-slate-800 mb-6 shadow-inner"><label className="text-[10px] text-slate-600 font-bold ml-1 mb-2 block uppercase tracking-widest font-black">Tutarı Yazınız</label><input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" className="w-full bg-transparent text-5xl font-mono text-white text-center outline-none tracking-tighter font-mono font-black" autoFocus /></div>
...
function FinancePanel({ user, players, currentTurn, handleSavings, calculateDebt, payDebt, handleLoan }) {
  ...
  return (
    <div className="animate-in fade-in space-y-6 pb-20 bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-2xl">
      <div className="flex bg-slate-950 p-1 rounded-2xl border border-slate-800 shadow-inner font-black uppercase tracking-widest font-black"><button type="button" onClick={() => setActiveSubTab('loan')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${activeSubTab === 'loan' ? 'bg-slate-800 text-white shadow-md border border-slate-700' : 'text-slate-600'}`}><HandCoins size={16}/> BORÇLARIM</button><button type="button" onClick={() => setActiveSubTab('savings')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${activeSubTab === 'savings' ? 'bg-amber-900/30 text-amber-500 shadow-md border border-amber-900/50' : 'text-slate-600'}`}><PiggyBank size={16}/> VADELİ</button></div>
      {activeSubTab === 'savings' && <div className="bg-gradient-to-br from-amber-950/40 to-slate-900 border border-amber-900/30 rounded-3xl p-6 shadow-xl"><div className="mb-6"><span className="text-amber-500 text-[10px] font-bold uppercase tracking-[0.3em] font-bold uppercase tracking-widest font-black">Vadeli Hesabım</span><div className="text-4xl font-mono text-white font-black mt-1 font-mono font-black">{user.savings.toLocaleString()} <span className="text-xl font-bold">TL</span></div></div>{user.savings > 0 ? <div className="bg-slate-950/60 rounded-2xl p-4 border border-slate-800 shadow-inner"><div className="flex justify-between items-center mb-2 font-black"><span className="text-slate-500 text-[9px] font-bold uppercase font-bold">Durum</span><span className="text-white font-mono font-bold text-xs uppercase tracking-tighter font-mono">{turnsPassed} / {(Math.floor(turnsPassed / 5) + 1) * 5} Tur Beklendi</span></div><div className="w-full bg-slate-900 rounded-full h-2 mb-4 overflow-hidden border border-slate-800"><div className={`h-full rounded-full transition-all duration-1000 ${isMilestone ? 'bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.4)]' : 'bg-amber-500'}`} style={{ width: `${(turnsPassed % 5 === 0 && turnsPassed > 0 ? 100 : (turnsPassed % 5) * 20)}%` }}></div></div><div className="flex justify-between items-center font-black"><span className={isMilestone ? "text-green-400 font-bold text-[10px] font-bold" : "text-slate-500 text-[10px] font-bold"}>{isMilestone ? "✅ KAZANÇ HAZIR" : "⌛ TUR BEKLENİYOR"}</span><span className={isMilestone ? "text-green-400 font-bold font-mono text-xs font-mono font-bold" : "text-slate-600 font-mono text-xs font-mono font-bold"}>+{interestAmount.toLocaleString()} TL (%{interestRate})</span></div></div> : <div className="text-slate-500 text-xs italic bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 italic font-bold">Birikim yaparak her 5 turda bir %10 kazanç sağlayabilirsiniz. Erken çekerseniz sadece ana paranızı alırsınız.</div>}<div className="mt-6 border-t border-slate-800 pt-6">{user.savings === 0 ? <div className="flex gap-2"><input type="number" value={savingsAmount} onChange={e => setSavingsAmount(e.target.value)} placeholder="0" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-4 font-mono text-white text-center outline-none focus:border-amber-500 shadow-inner font-mono font-black font-mono font-black" /><button type="button" onClick={() => { handleSavings(user.id, savingsAmount, 'DEPOSIT'); setSavingsAmount(''); }} className="bg-amber-600 text-white font-bold px-8 rounded-xl shadow-lg active:scale-95 transition-all text-xs tracking-widest uppercase font-bold">Yatır</button></div> : <button type="button" onClick={() => handleSavings(user.id, 0, 'WITHDRAW')} className={`w-full py-4 font-bold rounded-xl active:scale-95 transition-all flex flex-col items-center justify-center gap-1 shadow-lg ${isMilestone ? 'bg-green-600 text-white shadow-xl' : 'bg-slate-800 text-red-400 border border-red-900/20'}`}><span className="uppercase tracking-widest text-xs font-black">{isMilestone ? 'Kârı Çek' : 'Hesabı Kapat'}</span>{!isMilestone && <span className="text-[8px] opacity-60 font-bold font-bold">(Faiz Silinecektir!)</span>}</button>}</div></div>}
      {activeSubTab === 'loan' && <><div className="bg-gradient-to-br from-red-950/20 to-slate-900 border border-red-900/20 rounded-3xl p-6 shadow-xl"><div className="mb-6"><span className="text-red-500 text-[10px] font-bold uppercase tracking-[0.3em] font-black">Toplam Borç</span><div className="text-4xl font-mono text-white font-black mt-1 font-mono font-black font-black">{user.debt > 0 ? currentDebt.toLocaleString() : "0"} <span className="text-xl font-bold">TL</span></div></div>{user.debt > 0 ? <div className="relative z-10 font-black"><div className="bg-slate-950/60 rounded-2xl p-5 border border-red-900/10 mb-6 shadow-inner font-bold"><div className="flex justify-between text-xs mb-2 text-slate-400 uppercase tracking-tighter"><span>Ana Para</span><span className="font-mono text-white font-mono font-bold font-mono">{user.debt.toLocaleString()} TL</span></div><div className="flex justify-between text-xs mb-2 text-slate-400 uppercase tracking-tighter font-black"><span>Geçen Süre</span><span className="font-mono text-white font-mono font-bold font-mono">{currentTurn - user.debtStartTurn} Tur</span></div><div className="flex justify-between text-xs text-red-500 font-bold border-t border-slate-800 pt-2 mt-2 uppercase tracking-widest font-black"><span>Faiz Oranı</span><span className="font-mono font-mono font-bold font-mono">% {((currentTurn - user.debtStartTurn) >= 2) ? (currentTurn - user.debtStartTurn - 1) * 25 : 0}</span></div></div><button type="button" onClick={() => payDebt(user.id)} className="w-full py-4 bg-white text-red-900 font-black rounded-xl hover:bg-slate-100 active:scale-95 shadow-2xl tracking-[0.2em] uppercase text-xs font-bold font-black">Borcu Kapat</button></div> : <div className="text-slate-500 text-xs italic bg-slate-950/30 p-4 rounded-xl border border-slate-800/50 font-bold italic font-black uppercase tracking-widest font-black">Şu an borcunuz bulunmuyor.</div>}</div><div className="bg-slate-950/50 border border-slate-800 rounded-3xl p-6 shadow-xl shadow-inner font-black font-black"><h4 className="text-white font-bold mb-4 flex items-center gap-2 text-[10px] uppercase tracking-[0.25em] font-black tracking-widest uppercase font-black"><ArrowRightLeft size={14} className="text-indigo-400"/> Borç Ver</h4><div className="flex flex-col gap-3 font-bold uppercase"><select className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white text-xs outline-none focus:border-indigo-500 font-black uppercase font-black" value={loanTarget} onChange={(e) => setLoanTarget(e.target.value)}><option value="">Arkadaş Seç</option>{players.filter(p => p.id !== user.id && !p.isBankerOnly).map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}</select><div className="flex gap-2 font-bold uppercase font-black"><input type="number" placeholder="0" value={loanAmount} onChange={(e) => setLoanAmount(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-white text-lg font-mono outline-none focus:border-indigo-500 shadow-inner font-mono font-black font-black" /><button type="button" onClick={() => { if(loanTarget && loanAmount) { handleLoan(user.id, parseInt(loanTarget), loanAmount); setLoanAmount(''); setLoanTarget(''); } }} disabled={!loanTarget || !loanAmount} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-8 rounded-xl shadow-lg active:scale-95 transition-all uppercase text-[10px] tracking-widest font-black font-black font-bold uppercase">Borç Ver</button></div></div></div></>}
    </div>
  );
}
...
function BankerControls({ players, onTransaction, handleSalary, handleLoan }) {
  ...
  return (
    <div className="animate-in fade-in pb-20">
      <div className="flex items-center gap-3 mb-6 p-6 bg-gradient-to-r from-amber-600 to-orange-500 rounded-[2rem] shadow-xl text-white ring-2 ring-white/10 shadow-inner"><div className="bg-white/20 p-3 rounded-2xl"><ShieldAlert size={32} /></div><div><h3 className="font-black text-xl leading-tight uppercase tracking-tighter font-black">Bankacı Yönetimi</h3><p className="text-[10px] text-white/70 uppercase font-bold tracking-[0.2em] font-bold">İşlemleri Onayla</p></div></div>
      <div className="grid grid-cols-1 gap-4 font-black">{players.filter(p => !p.isBankerOnly).map(p => (<div key={p.id} className="bg-slate-900 border border-slate-800 p-5 rounded-3xl flex items-center justify-between hover:border-slate-600 transition-all shadow-lg ring-1 ring-slate-800 shadow-inner"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-slate-800 border-2 border-slate-700 flex items-center justify-center overflow-hidden shadow-inner">{(() => { const token = GAME_TOKENS.find(t => t.id === p.token); return token?.img ? <img src={token.img} alt="" className="w-full h-full object-cover"/> : token?.icon; })()}</div><div><div className="font-bold text-white flex items-center gap-2 text-base uppercase font-black">{p.name}<span className="text-[8px] bg-slate-800 px-2 py-0.5 rounded-full text-teal-400 border border-slate-700 font-mono uppercase font-black tracking-widest font-mono">T{p.turn}</span></div><div className="text-[10px] font-mono text-slate-500 flex items-center gap-1 mt-0.5 uppercase tracking-tighter italic font-black font-mono font-bold"><EyeOff size={12} /> Bakiye Gizlendi</div></div></div><button onClick={() => setSelectedPlayer(p)} className="bg-slate-800 hover:bg-slate-700 text-[10px] px-6 py-3 rounded-xl text-white border border-slate-700 font-black uppercase tracking-widest active:scale-95 shadow-md font-black">Yönet</button></div>))}</div>
      {selectedPlayer && <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-md flex items-end sm:items-center justify-center p-4"><div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl overflow-y-auto max-h-[90vh] shadow-inner"><div className="text-center mb-8"><div className="inline-block w-24 h-24 bg-slate-800 rounded-full mb-4 border-4 border-slate-700 overflow-hidden shadow-2xl">{(() => { const token = GAME_TOKENS.find(t => t.id === selectedPlayer.token); return token?.img ? <img src={token.img} alt="" className="w-full h-full object-cover"/> : token?.icon; })()}</div><h3 className="text-3xl font-black text-white tracking-tighter uppercase font-black">{selectedPlayer.name}</h3><div className="flex items-center justify-center gap-2 mt-2 font-black uppercase"><span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest font-black">Durum:</span><span className="text-teal-500 font-mono font-bold text-xs uppercase bg-slate-800 px-3 py-1 rounded-full font-mono font-bold font-black">Tur {selectedPlayer.turn}</span></div></div><button type="button" onClick={() => {handleSalary(selectedPlayer.id); setSelectedPlayer(null);}} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black py-5 rounded-2xl mb-8 shadow-2xl active:scale-95 transition-all uppercase tracking-[0.15em] flex items-center justify-center gap-3 ring-1 ring-white/10 font-black font-black uppercase font-black"><Briefcase size={20} /> Maaş & Tur Atlat</button><div className="bg-slate-950/60 p-6 rounded-[2rem] border border-slate-800 mb-6 shadow-inner uppercase tracking-widest font-black font-black font-black"><h4 className="text-[10px] font-black text-slate-500 mb-4 uppercase tracking-[0.2em] text-center font-bold tracking-widest font-black uppercase">Manuel İşlem</h4><div className="grid grid-cols-2 gap-3 mb-4 font-bold uppercase font-black font-black"><button type="button" onClick={() => setType('GIVE')} className={`py-3 rounded-xl border text-[10px] font-black transition-all uppercase tracking-widest font-black ${type === 'GIVE' ? 'bg-teal-600 border-teal-400 text-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-600'}`}>Para Ekle</button><button type="button" onClick={() => setType('TAKE')} className={`py-3 rounded-xl border text-[10px] font-black transition-all uppercase tracking-widest font-black ${type === 'TAKE' ? 'bg-red-600 border-red-400 text-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-600'}`}>Para Çek</button></div><div className="flex gap-2 font-bold uppercase font-black font-black font-black font-black"><input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-4 text-2xl font-mono text-white text-center outline-none shadow-inner font-black font-mono font-bold" /><button onClick={execute} className="bg-white text-slate-900 font-black rounded-xl px-6 hover:bg-slate-100 uppercase text-[10px] shadow-lg active:scale-90 tracking-widest font-black font-black font-black font-black font-black uppercase font-black">Ok</button></div></div><div className="bg-red-950/10 p-6 rounded-[2rem] border border-red-900/20 mb-4 shadow-inner uppercase font-black font-black font-black font-black font-black uppercase font-black font-black"><h4 className="text-[10px] font-black text-red-500/60 mb-4 uppercase tracking-[0.2em] text-center font-bold tracking-widest font-black uppercase font-black font-black">Banka Kredisi Ver</h4><div className="flex gap-2 font-bold uppercase font-black font-black font-black font-black uppercase font-black font-black"><input type="number" value={loanAmount} onChange={e => setLoanAmount(e.target.value)} placeholder="0" className="w-full bg-slate-900 border border-red-900/10 rounded-xl px-4 py-3 text-lg font-mono text-white text-center outline-none font-mono font-bold font-black" /><button type="button" onClick={() => { if(loanAmount) { handleLoan('BANKA', selectedPlayer.id, loanAmount); setLoanAmount(''); setSelectedPlayer(null); } }} className="bg-red-600 hover:bg-red-500 text-white font-black px-6 rounded-xl text-[10px] uppercase shadow-lg active:scale-90 tracking-widest font-black uppercase font-black font-black font-black font-black uppercase font-black font-black font-black">Kredi</button></div></div><button type="button" onClick={() => setSelectedPlayer(null)} className="w-full py-5 text-slate-500 text-xs font-black rounded-2xl mt-4 hover:text-white transition-all uppercase tracking-[0.4em] font-black font-black font-black font-black uppercase font-black font-black font-black">Paneli Kapat</button></div></div>}
    </div>
  );
}

function TransactionFeed({ transactions }) {
  if (transactions.length === 0) return <div className="text-center text-slate-600 py-16 text-sm italic animate-pulse tracking-widest uppercase font-bold font-bold italic uppercase tracking-widest font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black">Henüz hareket yok...</div>;
  return (
    <div className="space-y-4 pb-24 animate-in fade-in uppercase font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black"><h3 className="text-[10px] text-slate-600 font-black uppercase tracking-[0.4em] pl-2 mb-6 font-bold font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black">İşlem Geçmişi</h3>{transactions.map(t => (<div key={t.id} className={`bg-slate-900/70 border border-slate-800 p-5 rounded-3xl flex items-center justify-between shadow-xl backdrop-blur-sm group transition-all hover:bg-slate-800 ${t.type.includes('İFLAS') ? 'border-red-900/30 bg-red-950/10' : ''} shadow-inner font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black`}><div className="flex items-center gap-4 uppercase font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black"><div className={`p-3 rounded-2xl shadow-inner ${t.from.includes('BANKA') || t.from === 'SİSTEM' ? 'bg-teal-500/10 text-teal-500' : (t.type.includes('İFLAS') ? 'bg-red-500/10 text-red-500' : 'bg-fuchsia-500/10 text-fuchsia-500')}`}>{t.from.includes('BANKA') || t.from === 'SİSTEM' ? <Landmark size={20} /> : (t.type.includes('İFLAS') ? <Skull size={20}/> : <LayoutGrid size={20} />)}</div><div><div className="text-sm font-black text-white mb-0.5 group-hover:text-teal-400 transition-colors uppercase tracking-tighter font-black uppercase font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black">{t.from} <span className="text-slate-600 mx-1 font-bold">➝</span> {t.to}</div><div className="text-[9px] text-slate-500 uppercase tracking-[0.2em] font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black">{t.type} • {t.time}</div></div></div><div className={`font-mono font-black text-base font-mono font-black font-mono font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black`}>{t.amount > 0 ? t.amount.toLocaleString() : ''} {t.amount > 0 && 'TL'}</div></div>))}</div>
  );
}